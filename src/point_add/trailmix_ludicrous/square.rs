...
use super::arith::{
    self, controlled_mod_add_k, controlled_mod_sub_k, cuccaro_carry, mod_add_lowpeak,
    mod_add_shifted_low, mod_sub, mod_sub_shifted_low, F_SECP256K1, LSBS,
};
use super::{BExt, B};
...
fn controlled_apply_full_width(
    circ: &mut B,
    ctrl: &QubitId,
    operand: &[QubitId],
    output_reg: &[QubitId],
    ...
    match op {
        ShiftOp::Add => controlled_mod_add_k(circ, ctrl, operand, output_reg, None, None),
        ShiftOp::Sub => controlled_mod_sub_k(circ, ctrl, operand, output_reg),
    }
}
...
fn controlled_apply_unshifted_value(
    circ: &mut B,
    ctrl: &QubitId,
    value: &[QubitId],
    output_reg: &[QubitId],
    ...
    operand.extend_from_slice(value);
    operand.extend_from_slice(&pads);
    controlled_apply_full_width(circ, ctrl, &operand, output_reg, op);
    free_zeroes(circ, pads);
}
...
fn controlled_apply_shifted_value_direct(
    circ: &mut B,
    ctrl: &QubitId,
    value: &[QubitId],
    output_reg: &[QubitId],
...
fn controlled_add_f_window_shifted(circ: &mut B, ctrl: &QubitId, reg: &[QubitId], offset: usize) {
    let f_bytes = F_SECP256K1.to_le_bytes();
    arith::add_f_window_pub(circ, ctrl, &reg[offset..], LSBS, &f_bytes, None);
}

fn controlled_sub_f_window_shifted(circ: &mut B, ctrl: &QubitId, reg: &[QubitId], offset: usize) {
    for q in &reg[offset..offset + LSBS] {
        circ.x(*q);
    }
    controlled_add_f_window_shifted(circ, ctrl, reg, offset);
    for q in &reg[offset..offset + LSBS] {
        circ.x(*q);
    }
}

fn controlled_apply_shifted_hi_term(
    circ: &mut B,
    ctrl: &QubitId,
    hi: &[QubitId],
    output_reg: &[QubitId],
    ...
    if shift == 0 {
        controlled_apply_full_width(circ, ctrl, hi, output_reg, op);
        return;
    }

    controlled_apply_shifted_value_direct(circ, ctrl, &hi[..n - shift], output_reg, shift, op);

    for t in 0..shift {
        let gate = circ.alloc_qubit();
        circ.ccx(*ctrl, hi[n - shift + t], gate);
        match op {
            ShiftOp::Add => controlled_add_f_window_shifted(circ, &gate, output_reg, t),
            ShiftOp::Sub => controlled_sub_f_window_shifted(circ, &gate, output_reg, t),
        }
        circ.ccx(*ctrl, hi[n - shift + t], gate);
...
fn controlled_apply_f_times_value(
    circ: &mut B,
    ctrl: &QubitId,
    value: &[QubitId],
    output_reg: &[QubitId],
    ...
    for &(shift, sub_f_op) in &F_NAF_TERMS {
        ...
        if value.len() == N {
            controlled_apply_shifted_hi_term(circ, ctrl, value, output_reg, shift, term_op);
        } else {
            controlled_apply_shifted_value_direct(circ, ctrl, value, output_reg, shift, term_op);
        }
    }
}
...
fn controlled_apply_shifted_128(
    circ: &mut B,
    ctrl: &QubitId,
    value: &[QubitId],
    output_reg: &[QubitId],
    ...
    if value.len() > 128 {
        controlled_apply_f_times_value(circ, ctrl, &value[128..], output_reg, op);
    }
}
...
/// Controlled `output_reg -= lambda^2 mod q`.
///
/// The product construction and reverse construction are unchanged from
/// [`mod_square_sub_pm_secp256k1_symmetric`]. Only the modular applications to
/// `output_reg` are gated by `ctrl`, which is the Andre/Qarton `c = (i != 0)`
/// branch bit.
pub fn controlled_mod_square_sub_pm_secp256k1_symmetric(
    ...
    symmetric_square_into_prod(circ, &sum, &mut c_prod);
    circ.set_phase("square_c_sum_apply_shifted_128_sub");
    controlled_apply_shifted_128(circ, ctrl, &c_prod, output_reg, ShiftOp::Sub);
    circ.set_phase("square_c_sum_unbuild");
    symmetric_square_into_prod_reverse(circ, &sum, c_prod);
    ...
    symmetric_square_into_prod(circ, lo, &mut a_prod);
    circ.set_phase("square_a_lo_apply_unshifted_sub");
    controlled_apply_unshifted_value(circ, ctrl, &a_prod, output_reg, ShiftOp::Sub);
    circ.set_phase("square_a_lo_apply_shifted_128_add");
    controlled_apply_shifted_128(circ, ctrl, &a_prod, output_reg, ShiftOp::Add);
    circ.set_phase("square_a_lo_unbuild");
    symmetric_square_into_prod_reverse(circ, lo, a_prod);
    ...
    symmetric_square_into_prod(circ, hi, &mut b_prod);
    circ.set_phase("square_b_hi_apply_shifted_128_add");
    controlled_apply_shifted_128(circ, ctrl, &b_prod, output_reg, ShiftOp::Add);
    circ.set_phase("square_b_hi_apply_f_times_sub");
    controlled_apply_f_times_value(circ, ctrl, &b_prod, output_reg, ShiftOp::Sub);
    circ.set_phase("square_b_hi_unbuild");
    symmetric_square_into_prod_reverse(circ, hi, b_prod);
